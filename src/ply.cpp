...
bool Ply::ParseHeader(std::ifstream& plyFile)
{
    ...
    {
        spdlog::error("Unexpected error reading next line");
        return false;
    }
    if (token1 != "ply")
    {
        spdlog::error("Invalid ply file");
        return false;
    }
    ...
    {
        spdlog::error("Invalid ply file, expected format");
        return false;
    }
    if (token1 != "format binary_little_endian 1.0")
    {
        spdlog::error("Unsupported ply file, only binary_little_endian supported");
        return false;
    }
    ...
    {
        spdlog::error("Invalid ply file, expected \"element vertex {number}\"");
        return false;
    }
    ...
    while (true)
    {
        if (!GetNextPlyLine(plyFile, line))
        {
            spdlog::error("unexpected error reading line");
            return false;
        }
        ...
        if (token1 != "property")
        {
            spdlog::error("Invalid header, expected property");
            return false;
        }
        ...
        {
            spdlog::error("Unsupported type \"{}\" for property \"{}\"\n", token2, token3);
            return false;
        }
    }
