...
bool GaussianCloud::ImportPly(const std::string& plyFilename)
{
...

    {
        ...
        {
            spdlog::error("Error parsing ply file \"{}\"\n", plyFilename);
            return false;
        }
    }
...

    {
        ...
        {
            spdlog::error("Error parsing ply file \"{}\", missing position property\n", plyFilename);
        }
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                spdlog::error("Error parsing ply file \"{}\", missing f_dc property\n", plyFilename);
            }
        }
        ...
        if (opt.importFullSH)
        {
            ...
            for (int i = 0; i < 45; i++)
            {
                if (!ply.GetProperty("f_rest_" + std::to_string(i), props.f_rest[i]))
                {
                    ...
                    spdlog::warn("PLY file \"{}\", missing f_rest property\n", plyFilename);
                    hasFullSH = false;
                    break;
                }
            }
        ...
        {
            spdlog::error("Error parsing ply file \"{}\", missing opacity property\n", plyFilename);
        }
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                spdlog::error("Error parsing ply file \"{}\", missing scale property\n", plyFilename);
            }
        }
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                spdlog::error("Error parsing ply file \"{}\", missing rot property\n", plyFilename);
            }
        }
